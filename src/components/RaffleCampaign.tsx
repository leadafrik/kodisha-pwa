...
const RaffleCampaign: React.FC = () => {
  ...
  return (
    <section
      id="raffle-campaign"
      className="rounded-3xl border border-[#c9d7c4] bg-[#f5f9f2] p-6 shadow-sm space-y-6"
    >
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="inline-flex items-center gap-2 rounded-full bg-[#6b856b] px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-white">
        ...
        <div className="rounded-2xl border border-[#c9d7c4] bg-white px-4 py-3 text-sm">
      ...
      <div className="space-y-2">
        <div className="relative h-3 overflow-hidden rounded-full bg-slate-100">
          <div
            className="absolute inset-y-0 left-0 bg-[#6b856b] transition-all duration-500"
            style={{ width: `${percent}%` }}
          />
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {["1. Sign up", "2. Get verified", "3. Post a listing"].map((step) => (
          ...
          >
            <CheckCircle2 className="h-5 w-5 text-[#6b856b]" />
            <span>{step}</span>
          </div>
        ))}
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_auto]">
        <div className="rounded-2xl border border-dashed border-[#c9d7c4] bg-[#edf4e9] p-4">
          <div className="flex items-center gap-2 text-sm font-semibold text-[#586f58]">
          ...
          <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2">
            ...
            <button
              ...
              className={`rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-white ${
                inviteCode ? "bg-[#6b856b] hover:bg-[#586f58]" : "bg-slate-300"
              }`}
            >
              Copy
            </button>
          </div>
          {copyFeedback && <p className="text-xs text-[#586f58]">{copyFeedback}</p>}
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white/90 p-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="rounded-full bg-[#e6f0e0] px-3 py-1 text-xs font-semibold text-[#586f58]">
